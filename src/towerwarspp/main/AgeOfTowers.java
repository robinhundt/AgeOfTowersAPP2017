...
import towerwarspp.main.game.Save;
...
public class AgeOfTowers {
    ...
    private Player createPlayer(PlayerType playerType) {
        ...
        switch (playerType) {
            case HUMAN:
                player = new HumanPlayer(requestable);
                break;
            case RANDOM_AI:
                player = new RndPlayer();
                break;
            case SIMPLE_AI:
                player = new SimplePlayer();
                break;
            case ADVANCED_AI_1:
                player = new Adv1Player();
                break;
            case ADVANCED_AI_2:
                player = createAdv2Player();
                break;
            case REMOTE:
                player = getRemotePlayer();
                break;
            default:
                System.out.println("Unsupported PlayerType.");
                System.exit(1);
        }
        return player;
    }
    ...
    private static String helpOutput() {
        return "Starts a new game of TowerWarsPP with the given settings. \n" +
                ...
                "--fair     \t will cause the AI to spent as much time deciding on a move as long as the nemy took";
